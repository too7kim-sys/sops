...
public class EgovChangeController {
    ...
    public String detail(@PathVariable Long chgId, Model model,
        ...
        // CAB 심의 대상이 아니면(결재라인에 검토 없음) 검토·승인을 자동 완료 —
        ...
        if (change != null
            ...
            apprService.autoReviewApprove("CHANGE", chgId, loginUser.getUsername(),
                    "자동 검토·승인(CAB 심의 대상 아님 · 검토 결재라인 없음)");
        ...
        // CAB 심의는 결재라인에 검토(REVIEW)가 있을 때만 수행 — 없으면 CAB 폼 미노출(자동 검토·승인)
        model.addAttribute("cabRequired", change != null && isCabRequired(chgId));
    ...
    /** CAB 심의 대상 여부 — 결재라인에 검토(REVIEW)가 있을 때만 수행, 없으면 자동 검토·승인 */
    private boolean isCabRequired(Long chgId) {
        return hasReviewer(chgId);
    }
    ...
    public String insert(@ModelAttribute ChangeVO changeVO,
        ...
        // CAB 심의는 결재라인에 검토(REVIEW)가 있을 때만 수행, 없으면 CAB 없이 자동 검토·승인
        if (!isCabRequired(changeVO.getChgId())) {
